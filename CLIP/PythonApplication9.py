...
def calculate_clip_score(path_to_image, prompt):
    ...
    model, preprocess = clip.load('RN50x64')  #'ViT-L/14@336px', 'RN50x64': Returns the model and the TorchVision transform needed by the mode
    try:    
...
def main():
    ...
    ############################################################################
    # The following must be done a function which will be return Mean and Median
    # We are going to compute the Mean and Median of CLIP score for each Category/Task Type Per Model
    #print("list:", image_text_values_list)
