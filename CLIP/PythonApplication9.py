...
my_benchmark_tuple = (
    ...
    ("coloring", "Two pink football balls and three green basketball balls on a bench", 12),
...
def calculate_clip_score(path_to_image, prompt):
    ...
    model, preprocess = clip.load('ViT-L/14@336px')  #ViT-B/32,ViT-L/14: Returns the model and the TorchVision transform needed by the mode
    try:    
...
def main():
    ...
    file_path = "C:/Users/E.Mazarakis/Desktop/clip_scores_file.csv"
            ...
            for j in range( len(img_sizes) ):           # For the 2 sizes (512, 1024)
                for k in range( len(tasks) ):           # For the 6 task type
                    for l in range( NUM_OF_IMAGES ):    # For the 12 images
                        ...
                        print(image_file_path + "|" + text + "|" + str(format(CLIP_SCORE, '.4f')))
                        ...
                        writer.writerow([model, size, task_type, photo, prompt, score])  
                    print("------------------------------------------------------------------------")
    ...
    print("CLIP scores computed on:  ", duration, "seconds")
